...
async function loadFiles(path = '') {
  ...
  try {
    ...
    if (data.folders.length === 0 && data.files.length === 0) {
      ...
      emptyRow.innerHTML = '<td colspan="3" style="padding:18px 8px;opacity:0.8">Nada para mostrar..</td>';
      listElement.appendChild(emptyRow);
    }

  } catch (error) {
    console.error('Error fetching directory content:', error);
    listElement.innerHTML = '<tr><td colspan="3" style="color:red; padding:10px 8px;">Erro ào conectar ao servidor. Como a descrição diz, eu não consigo usar o servidor porque eu não tenho um domínio.</td></tr>';
  }
}
