...
async function loadFiles(path = '') {
  ...
  try {
    ...
    if (data.folders.length === 0 && data.files.length === 0) {
      ...
      emptyRow.innerHTML = '<td colspan="3" style="padding:18px 8px;opacity:0.8">Nothing to display..</td>';
      listElement.appendChild(emptyRow);
    }

  } catch (error) {
    console.error('Error fetching directory content:', error);
    listElement.innerHTML = '<tr><td colspan="3" style="color:red; padding:10px 8px;">Failed to connect to the file server. As the description says, I can\'t use the file server because I don\'t have a domain.</td></tr>';
  }
}
